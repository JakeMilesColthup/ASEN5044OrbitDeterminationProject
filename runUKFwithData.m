function [xUKF, PUKF, SUKF] = runUKFwithData(ydata, Qtrue, Rtrue, xplus0, Pplus0, nTimesteps, deltaT, mu, nStations, tvec, RE, omegaE)
...
for ii = 2:nTimesteps
    ...
    if ~isempty(visibleStationIds)
        ...
        gammaPts = zeros(length(yDataVec),2*n+1);
        ...
        [~, visibleStationIds] = checkVisibleStationsNoisyMeas(nStations, RE, omegaE, tvec(kp1), yDataVec36);
        ...
        assert(length(yDataVec) == size(gammaPts,1), 'ERROR: Size of yData does not match size of gammaPts')

        % Combine resultant pts to get mean
        sum = zeros(length(yDataVec),1);
        for zz = 1:2*n+1
        ...
        % Combine resultant pts to get cov
        sum = zeros(length(yDataVec),length(yDataVec));
        for zz = 1:2*n+1
        ...
        % Calculate new state
        xhatkp1Plus = xhatkp1Minus + Kkp1 * (yDataVec - yHatVec);

        % Calculate Pkp1plus
        PUKF(:,:,kp1) = PUKFMinus(:,:,kp1) - Kkp1 * Pyy{kp1} * Kkp1';

        % Calculate yError (or usual ey,k)
%         yErrorUKF{kp1} = yDataVec - yHatVec;
