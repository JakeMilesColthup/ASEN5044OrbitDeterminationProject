function [xEKF, PEKF, SEKF] = runEKFwithData(ydata, Qtrue, Rtrue, xplus0, Pplus0, deltaT, RE, omegaE, mu, tvec, nTimesteps, nStations)
% Run EKF once for all timesteps. Simulates one complete simulation based on measurements.
% Inputs are nominal trajectory (NOT used in the EKF itself, just to calculate error after), ydata, Qtrue and Rtrue from Canvas data,
...
%     [stationVec, visibleStationIds] = checkVisibleStations(nStations, RE, omegaE, xhatkp1Minus, tvec(kp1));
    ...
    [stationVec, visibleStationIds] = checkVisibleStationsNoisyMeas(nStations, RE, omegaE, tvec(kp1), yDataVec36);
    ...
    if ~isempty(visibleStationIds)
        ...
        % Extract y data into one stacked vec at this timestep
        [yHatVec, ~] = reformatYNomWithNoisy(yhatkp1Minus, yDataVec36);

        % Ensure that size of yData here matches size of y combined
        assert(length(yDataVec) == size(Hcombined,1), 'ERROR: Size of yData does not match size of Hcombined')

        % Calculate NL meas. innovation (actual data minus predicted)
        etildeykp1 = yDataVec - yHatVec;
